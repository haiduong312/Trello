...
export const columnService = {
    async getColumnsByColumnId(columnId: string): Promise<IColumn[]> {
        const { data, error } = await supabase
            .from("columns")
            .select("*")
            .eq("id", columnId)
    ...
    async getColumnsByBoardId(boardId: string): Promise<IColumn[]> {
        const { data, error } = await supabase
            .from("columns")
            .select("*")
            .eq("board_id", boardId)
    ...
    async updateColumnPositions(
        ...
        const { error } = await supabase.rpc("update_column_positions", {
            positions,
        });
