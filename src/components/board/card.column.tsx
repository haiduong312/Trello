...
interface IProps {
  ...
  handleCardOk: (columnId: string, cardTitle: string) => void; // Hàm thêm card
  activeAddCardColumnId: string | null; // ID cột đang được mở input thêm card
  setActiveAddCardColumnId: React.Dispatch<React.SetStateAction<string | null>>; // Hàm thay đổi ID cột đang mở input
}
      ...
      {activeAddCardColumnId === col.id ? (
        <div>
          ...
          >
            ...
            <div
              className="close-btn"
              onClick={() => setActiveAddCardColumnId(null)}
            >
              <CloseOutlined />
            </div>
          </div>
